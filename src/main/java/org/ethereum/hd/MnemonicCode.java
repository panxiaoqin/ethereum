...
//    private static final String BIP39_ENGLISH_RESOURCE_NAME = "mnemonic/wordlist/zh_cn.txt";
...
//    }
    ...
    public MnemonicCode(String[] wordList) {
        INSTANCE = this;
...
//        }
    ...
    public static byte[] toSeed(List<String> words, String passphrase) {
        ...
        //
        String pass = SPACE_JOINER.join(words);
    ...
    public byte[] toEntropy(List<String> words) {
        if (words.size() % 3 > 0)
            throw new RuntimeException("Word list size must be multiple of three words.");
    ...
    public List<String> toMnemonic(byte[] entropy) {
        if (entropy.length % 4 > 0)
            throw new RuntimeException("Entropy length not multiple of 32 bits.");

        if (entropy.length == 0)
            throw new RuntimeException("Entropy is empty.");
